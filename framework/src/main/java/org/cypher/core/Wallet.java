...
import static org.cypher.common.utils.Commons.getAssetIssueStoreFinal;
import static org.cypher.common.utils.Commons.getExchangeStoreFinal;
import static org.cypher.common.utils.WalletUtil.isConstant;
...
import static org.cypher.core.config.Parameter.DatabaseConstants.EXCHANGE_COUNT_LIMIT_MAX;
import static org.cypher.core.config.Parameter.DatabaseConstants.MARKET_COUNT_LIMIT_MAX;
import static org.cypher.core.config.Parameter.DatabaseConstants.PROPOSAL_COUNT_LIMIT_MAX;
import static org.cypher.core.services.jsonrpc.JsonRpcApiUtil.parseEnergyFee;
...
import org.cypher.api.GrpcAPI.Address;
...
import org.cypher.api.GrpcAPI.Node;
...
import org.cypher.api.GrpcAPI.ShieldedAddressInfo;
...
import org.cypher.api.GrpcAPI.SpendNote;
...
import org.cypher.api.GrpcAPI.TransactionExtention.Builder;
...
import org.cypher.common.overlay.discover.node.NodeHandler;
...
import org.cypher.consensus.ConsensusDelegate;
import org.cypher.core.actuator.Actuator;
import org.cypher.core.actuator.ActuatorFactory;
import org.cypher.core.actuator.VMActuator;
...
import org.cypher.core.capsule.BlockBalanceTraceCapsule;
...
import org.cypher.core.capsule.IncrementalMerkleVoucherCapsule;
...
import org.cypher.core.capsule.TransactionRetCapsule;
import org.cypher.core.capsule.WitnessCapsule;
import org.cypher.core.capsule.utils.MarketUtils;
import org.cypher.core.capsule.utils.TransactionUtil;
import org.cypher.core.config.args.Args;
import org.cypher.core.db.BandwidthProcessor;
import org.cypher.core.db.BlockIndexStore;
import org.cypher.core.db.EnergyProcessor;
...
import org.cypher.core.exception.NonUniqueObjectException;
import org.cypher.core.exception.PermissionException;
import org.cypher.core.exception.SignatureFormatException;
...
import org.cypher.core.zen.ShieldedTRC20ParametersBuilder;
import org.cypher.core.zen.ShieldedTRC20ParametersBuilder.ShieldedTRC20ParametersType;
import org.cypher.core.zen.ZenTransactionBuilder;
...
import org.cypher.core.zen.note.Note;
import org.cypher.core.zen.note.NoteEncryption;
import org.cypher.core.zen.note.NoteEncryption.Encryption;
import org.cypher.core.zen.note.OutgoingPlaintext;
...
import org.cypher.protos.Protocol.MarketPrice;
...
import org.cypher.protos.contract.BalanceContract.BlockBalanceTrace;
import org.cypher.protos.contract.BalanceContract.TransferContract;
import org.cypher.protos.contract.ShieldContract.IncrementalMerkleTree;
...
import org.cypher.protos.contract.ShieldContract.ReceiveDescription;
...
public class Wallet {
  ...
  private boolean cypCacheEnable = Args.getInstance().isCypCacheEnable();
  ...
  private void setTransaction(TransactionCapsule cyp) {
    try {
      BlockId blockId = chainBaseManager.getHeadBlockId();
      if ("solid".equals(Args.getInstance().getCypReferenceBlock())) {
        blockId = chainBaseManager.getSolidBlockId();
      }
      cyp.setReference(blockId.getNum(), blockId.getBytes());
      long expiration = chainBaseManager.getHeadBlockTimeStamp() + Args.getInstance()
          .getCypExpirationTimeInMilliseconds();
      cyp.setExpiration(expiration);
      cyp.setTimestamp();
    } catch (Exception e) {
  ...
  private TransactionCapsule createTransactionCapsuleWithoutValidateWithTimeout(
      ...
      long timeout) {
    TransactionCapsule cyp = new TransactionCapsule(message, contractType);
    try {
      BlockId blockId = chainBaseManager.getHeadBlockId();
      if ("solid".equals(Args.getInstance().getCypReferenceBlock())) {
        blockId = chainBaseManager.getSolidBlockId();
  ...
  public TransactionCapsule createTransactionCapsule(com.google.protobuf.Message message,
    ...
    if (contractType != ContractType.CreateSmartContract
      ...
      List<Actuator> actList = ActuatorFactory.createActuator(cyp, chainBaseManager);
    ...
    if (contractType == ContractType.CreateSmartContract) {
      ...
      CreateSmartContract contract = ContractCapsule
          .getSmartContractFromTransaction(cyp.getInstance());
  ...
  public GrpcAPI.Return broadcastTransaction(Transaction signedTransaction) {
    ...
    TransactionCapsule cyp = new TransactionCapsule(signedTransaction);
    cyp.setTime(System.currentTimeMillis());
    Sha256Hash txID = cyp.getTransactionId();
    try {
      ...
      int num = cypherNetService.fastBroadcastTransaction(message);
  ...
  public TransactionApprovedList getTransactionApprovedList(Transaction cyp) {
    TransactionApprovedList.Builder tswBuilder = TransactionApprovedList.newBuilder();
    TransactionExtention.Builder cypExBuilder = TransactionExtention.newBuilder();
    cypExBuilder.setTransaction(cyp);
    cypExBuilder.setTxid(ByteString.copyFrom(Sha256Hash.hash(CommonParameter
        .getInstance().isECKeyCryptoEngine(), cyp.getRawData().toByteArray())));
    Return.Builder retBuilder = Return.newBuilder();
    retBuilder.setResult(true).setCode(response_code.SUCCESS);
    cypExBuilder.setResult(retBuilder);
    tswBuilder.setTransaction(cypExBuilder);
    TransactionApprovedList.Result.Builder resultBuilder = TransactionApprovedList.Result
        .newBuilder();
    try {
      Contract contract = cyp.getRawData().getContract(0);
      ...
      if (cyp.getSignatureCount() > 0) {
        List<ByteString> approveList = new ArrayList<ByteString>();
        byte[] hash = Sha256Hash.hash(CommonParameter
            .getInstance().isECKeyCryptoEngine(), cyp.getRawData().toByteArray());
        for (ByteString sig : cyp.getSignatureList()) {
          if (sig.size() < 65) {
  ...
  public AccountResourceMessage getAccountResource(ByteString accountAddress) {
    ...
    long totalNetLimit = chainBaseManager.getDynamicPropertiesStore().getTotalNetLimit();
    long totalNetWeight = chainBaseManager.getDynamicPropertiesStore().getTotalNetWeight();
    long totalCypherPowerWeight = chainBaseManager.getDynamicPropertiesStore()
        .getTotalCypherPowerWeight();
    ...
    long allCypherPowerUsage = accountCapsule.getCypherPowerUsage();
    long allCypherPower = accountCapsule.getAllCypherPower() / CYP_PRECISION;
    ...
    builder.setFreeNetUsed(accountCapsule.getFreeNetUsage())
        ...
        .setTotalNetLimit(totalNetLimit)
        .setTotalNetWeight(totalNetWeight)
        .setTotalCypherPowerWeight(totalCypherPowerWeight)
        .setEnergyLimit(energyLimit)
        .setEnergyUsed(accountCapsule.getAccountResource().getEnergyUsage())
        .setCypherPowerUsed(allCypherPowerUsage)
        .setCypherPowerLimit(allCypherPower)
  ...
  public DiversifierMessage getDiversifier() throws ZksnarkException {
    ...
    byte[] d;
    while (true) {
      d = org.cypher.keystore.Wallet.generateRandomBytes(Constant.ZC_DIVERSIFIER_SIZE);
  ...
  public NodeList listNodes() {
    ...
    nodeHandlerMap.entrySet().stream()
        .forEach(v -> {
          org.cypher.common.overlay.discover.node.Node node = v.getValue()
              .getNode();
  ...
  public Transaction deployContract(TransactionCapsule cypCap) {

    // do nothing, so can add some useful function later
    // cypCap contract para cacheUnpackValue has value

    return cypCap.getInstance();
  }

  public Transaction triggerContract(TriggerSmartContract
      triggerSmartContract,
      TransactionCapsule cypCap, Builder builder,
    ...
    if (isConstant(abi, selector)) {
      return callConstantContract(cypCap, builder, retBuilder);
    } else {
      return cypCap.getInstance();
    }
  }
  ...
  public Transaction triggerConstantContract(TriggerSmartContract triggerSmartContract,
      TransactionCapsule cypCap, Builder builder, Return.Builder retBuilder)
      throws ContractValidateException, ContractExeException, HeaderNotFound, VMIllegalException {
    ...
    if (triggerSmartContract.getContractAddress().isEmpty()) { // deploy contract
      ...
      cypCap = createTransactionCapsule(deployBuilder.build(), ContractType.CreateSmartContract);
    } else { // call contract
  ...
  public Transaction callConstantContract(TransactionCapsule cypCap,
    ...
    TransactionContext context = new TransactionContext(new BlockCapsule(headBlock), cypCap,
  ...
  private boolean isShieldedTRC20NoteSpent(GrpcAPI.Note note, long pos, byte[] ak,
    ...
    Return.Builder retBuilder = Return.newBuilder();
    TransactionExtention cypExt;

    try {
      TransactionCapsule cypCap = createTransactionCapsule(trigger,
          ContractType.TriggerSmartContract);
      Transaction cyp = triggerConstantContract(trigger, cypCap, cypExtBuilder, retBuilder);

      retBuilder.setResult(true).setCode(response_code.SUCCESS);
      cypExtBuilder.setTransaction(cyp);
      cypExtBuilder.setTxid(cypCap.getTransactionId().getByteString());
      cypExtBuilder.setResult(retBuilder);
      ...
      cypExt = cypExtBuilder.build();
    }

    String code = cypExt.getResult().getCode().toString();
    if ("SUCCESS".equals(code)) {
      List<ByteString> list = cypExt.getConstantResultList();
